...
var lJS = function(tag_id, params) {
  ...
  lJS.fn.snakeWindowsInit = function() {
    if(lJS.windowguide.length > 0) {// Check for objects defining snakewindows from the params
      ...
      lJS.snakewindows[2].x = Math.floor(lJS.canvas.width/2/window.devicePixelRatio
                               - lJS.snakewindows[2].width/2);
      lJS.snakewindows[2].y = Math.floor(lJS.canvas.height/window.devicePixelRatio
                               - lJS.snakewindows[2].width/2);
    }
  }
  ...
  lJS.fn.updateSnakes = function() {
    ...
    for(var i=0; i<lJS.snakes.length; i++) {
      ...
      if (lJS.snakes[i].deathCount > 0 && lJS.snakes[i].points.length > 3 && 
          Math.random() < 0.0025 && lJS.snakes.length < lJS.maxsnakes) {
      ...
      if (lJS.snakes[i].deathCount < 1 && lJS.snakes[i].deathCount > -1 && 
          (!lJS.dieoff || lJS.snakes.length < lJS.maxsnakes)) {
          ...
          if(lJS.grid.length-1 < point.x+k || point.x+k < 0 ||
            ...
            if(sn && lJS.grid[point.x+k][point.y+r] == sn.id) {
              ...
              if((lastpoint.x == point.x+k && lastpoint.y == point.y+r) ||
                (slastpoint != null && slastpoint.x == point.x+k && 
                  slastpoint.y == point.y+r)) {
           ...
           lJS.canvas.height/window.devicePixelRatio != height) {
            ...
            if(!resizeTimeout) {
              resizeTimeout = setTimeout(function() {
               ...
               // The actualResizeHandler will trigger 200ms after resize events stop firing
               }, 200);
            }
        }
      }
